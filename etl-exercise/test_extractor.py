...
def csv_file_2():
    with tempfile.NamedTemporaryFile(mode='w+', suffix='.csv', delete=False) as f:
        f.write("car_model,year_of_manufacture,price,fuel\n")
        f.write("Chevrolet,2019,23000,Hybrid\n")
        f.write("Hyundai,2015,15000,Gasoline\n")
        return f.name
...
def json_file():
    data = [
       ...
       # {"car": "BMW", "year_of_manufacture": 2021, "prices": 35000, "fuel": "Electric"},
       #{"car": "BMW", "year_of_manufacture": 2021, "prices": 35000, "fuel": "Electric"},
...
def xml_file():
    ...
    ET.SubElement(car2, "car").text = "Nissan"
...
def malformed_xml_file():
    root = ET.Element("cars")
    car1 = ET.SubElement(root, "car")
    ET.SubElement(car1, "wrong_tag").text = "test"
    
    car2 = ET.SubElement(root, "car")
    ET.SubElement(car2, "car_model").text = "Opel"
...
def test_extract_from_two_valid_csv_files(csv_file, csv_file_2):
    df1 = extract_from_csv(csv_file)
    df2 = extract_from_csv(csv_file_2)


    assert isinstance(df1, pd.DataFrame)
    assert df1.shape == (2, 4)  # 2 archivos x 2 filas cada uno
    assert set(df1.columns) == {'car_model', 'year_of_manufacture', 'price', 'fuel'}
    
    assert isinstance(df2, pd.DataFrame)
    assert df2.shape == (2, 4)  # 2 archivos x 2 filas cada uno
    assert set(df2.columns) == {'car_model', 'year_of_manufacture', 'price', 'fuel'}
...
def test_extract_from_invalid_json():
    ...
    # Asegura que se lanza ValueError al intentar procesarlo
